...
export class StakingService implements OnModuleInit {
  ...
  async updateHolders(): Promise<void> {
    try {
      ...
      const holders = await this.parseHolders(this.MAIN_ASSET);

      if (Object.keys(holders).length === 0) {
        this.logger.warn('No holders found for main asset, skipping update');
        return;
      }

      // More concise filtering
      const walletHolders = Object.fromEntries(
        wallets.map((w) => [w.publicKey, holders[w.publicKey] || 0]),
      );

      this.logger.log(
        `Updated balances for ${Object.keys(walletHolders).length} wallets`,
      );
      ...
      this.logger.error(
        `Unexpected error while updating holders: ${error.message}`,
      );
    }
  }
  ...
  async calculateRewards(): Promise<void> {
    ...
    try {
      ...
      const mainAssetHolders =
        (settings?.mainTokenHolders as Record<string, number>) || {};
      ...
      for (const stakingAsset of stakingAssets) {
        const asset = new Asset(
          stakingAsset.assetCode,
          stakingAsset.assetIssuer,
        );
        const holders = await this.parseHolders(asset);

        assetHoldersMap.set(stakingAsset.id, holders);
        this.logger.log(
          `${stakingAsset.assetCode}: ${Object.keys(holders).length} wallets with balance`,
        );
      }
      ...
      for (const wallet of wallets) {
        try {
          ...
          const walletBalance =
            mainAssetHolders[walletPublicKey] || wallet.balance || 0;

          this.logger.log(
            `w-${walletPublicKey}: MAIN_ASSET balance: ${walletBalance}`,
          );
          ...
          try {
            ...
            this.logger.error(
              `Invalid rewards data for wallet ${walletPublicKey}`,
            );
            rewards = [];
          }
          ...
          for (const stakingAsset of stakingAssets) {
            ...
            const balance =
              holders[walletPublicKey] || existingReward?.balance || 0;
            ...
            if (walletPercent > 0) {
              ...
              newReward.last_update_timestamp =
                existingReward?.last_update_timestamp || 0;

              // Calculate next reward time based on hour boundaries
              let nextRewardTime: number;
              if (newReward.last_update_timestamp > 0) {
                const lastUpdateDate = new Date(
                  newReward.last_update_timestamp * 1000,
                );
                const nextHour = new Date(lastUpdateDate);
                nextHour.setMinutes(0, 0, 0);
                nextHour.setHours(nextHour.getHours() + this.REWARD_INTERVAL);
                nextRewardTime = nextHour.getTime() / 1000;
              } else {
                // First time reward, set to next hour from current time
              ...
              if (
                (now >= nextRewardTime ||
                  newReward.last_update_timestamp === 0) &&
                walletBalance > 0
              ) {
                ...
                this.logger.log(
                  `w-${walletPublicKey}:a-#${stakingAsset.id}: ` +
                    `Updated reward to ${newReward.earned.toFixed(7)} ` +
                    `(last_update_timestamp: ${newReward.last_update_timestamp}, nextRewardTime: ${nextRewardTime})`,
                );
      ...
      if (rewardsToSend.length > 0) {
        ...
        if (sendingEnabled) {
          // Fire and forget - don't block the staking task
          this.distributorQueue
            .append(rewardsToSend, undefined, 'staking-rewards')
            .catch((error) => {
              this.logger.error(
                `Failed to append rewards to queue: ${error.message}`,
              );
            });
          this.logger.log(`Queued ${rewardsToSend.length} rewards for sending`);
        } else {
          this.logger.log(
    ...
    for (const stakingAsset of stakingAssets) {
      try {
        ...
        if (assetInfo) {
          ...
          this.logger.log(
            `${stakingAsset.assetCode} price updated to ${assetPrice}`,
          );
        }
      } catch (error) {
        this.logger.error(
  ...
  async runStakingLoop(): Promise<void> {
    while (true) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.TASK_INTERVAL * 1000),
      );
    }
  }
