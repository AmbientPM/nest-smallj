...
export class XRPService {
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    if (retryCount < maxRetries) {
                        ...
                        this.logger.error('XRPAPI: 429 Too Many Requests, max retries reached');
                        throw new XRPAPITooManyRequests('Too many requests');
                    }
                } else {
                    throw new XRPAPIError(`HTTP ${response.status}: ${response.statusText}`);
                }
            } catch (error) {
                if (error instanceof XRPAPIError || error instanceof XRPAPITooManyRequests) {
                    throw error;
                }
                ...
                if (retryCount < maxRetries) {
                    ...
                    this.logger.error(`XRPAPI: Request failed after ${maxRetries} attempts`);
    ...
    async getAccountTransactions(account: string, limit: number = 5): Promise<XRPTransaction[]> {
        try {
            const params = [
                {
                    account,
    ...
    async receive(
        ...
        try {
            ...
            const transactions = await this.getAccountTransactions(destinationAccountAddress, 20);
    ...
    async getBalance(account: string, currency: string = 'XRP', issuer?: string): Promise<number> {
        try {
            const params = [
                {
                    account,
            ...
            if (currency === 'XRP') {
                ...
                const linesResponse = await this.makeRequest('account_lines', linesParams);
                const lines = linesResponse.result?.lines || [];

                for (const line of lines) {
                    if (line.currency === currency && (!issuer || line.account === issuer)) {
